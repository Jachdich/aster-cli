...
pub struct GUI {
    ...
    pub mode: Mode,
    pub focus: Focus,
    pub screen: MouseTerminal<RawTerminal<std::io::Stdout>>,
    pub theme: Theme,
...
impl GUI {
    pub async fn new(
        ...
        for serv in config["servers"].as_array().expect("Invalid config file!") {
        ...
        let prompt = if config["uname"].is_null() || config["passwd"].is_null() {
            Some(Prompt::new(
                "Enter login details",
                vec![
                    PromptField::String {
                    ...
                    PromptField::U16 {
                        ...
                        default: config["sync_port"].as_u64().map(|u| u as u16),
                    },
                ],
